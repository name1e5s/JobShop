...
static inline void cp_onemach_time(onemach_times_t *new, onemach_times_t *origin);
...
/**
  Copy origin onemach_times struct to new
  By practice, change from memcpy to just write what we want to do
  is very important...
 */
static inline void cp_onemach_time(onemach_times_t *new, onemach_times_t *origin) {
	for (int i = 0; i < job_size; i++) {
		new->estime[i] = origin->estime[i];
		new->process_time[i] = origin->process_time[i];
		new->mhtime[i] = origin->mhtime[i];
	}
}
