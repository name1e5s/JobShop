...
static inline void cp_mo(mo_t *mew, mo_t *origin);
static inline void cp_seq(sequence_t *mew, sequence_t *origin);
static inline void cp_neighbor(neighbor_t *mew);
...
/**
  ...
  time after a mew machine is sequenced, all previously sequenced sequence
...
static inline void re_optimization_phase_1(sequence_t *seq, mo_t *machine_order, int* makespan) {
	...
	while (improvement > 0) {
		...
		for (int i = 0; i < machine_order->size; i++) {
			...
			if (terminate_flag) {
				...
				*makespan = eval(seq);
                ...
                mewmakespan = eval(seq);
                if (mewmakespan < oldmakespan)
                    *makespan = mewmakespan;
				else {
					set_seq(seq, machine_order->machines[i], old_order);
                    mewmakespan = oldmakespan;
				}
			}
		}
        ...
        improvement = oldmakespan - mewmakespan;
        oldmakespan = mewmakespan;
	}
}
 ...
 */
static inline void cp_mo(mo_t *mew, mo_t *origin) {
    mew->size = origin->size;
	for (int i = 0; i < machine_size; i++)
        mew->machines[i] = origin->machines[i];
}
...
static inline void cp_neighbor(neighbor_t *mew) {
	for (int i = 0; i < job_size; i++)
		for (int j = 0; j < machine_size; j++) {
            mew[i].next[j] = job[i].next[j];
            mew[i].prev[j] = job[i].prev[j];
		}
}
